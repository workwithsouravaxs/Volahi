...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 select-none w-full">
      {/* Announcement Bar */}
      <div className="bg-[#482922] text-white text-[9px] md:text-[10px] tracking-[0.18em] uppercase flex items-center justify-center h-8 font-semibold font-heading text-center px-4 w-full">
        Free Shipping Worldwide on Orders Over ₹5,000
      </div>

      <nav className={`transition-all duration-500 font-heading text-primary bg-white border-b border-[#E4DFDE] flex items-center h-16 md:h-20 ${
        isScrolled ? 'shadow-[0_4px_18px_rgba(72,41,34,0.06)]' : ''
      }`}>
        <div className="max-w-7xl mx-auto px-4 w-full relative flex items-center justify-between h-full">
          ...
          <div className="flex items-center gap-4 md:gap-16 h-full">
            <button 
              className="md:hidden p-1 text-primary hover:text-accent transition-colors relative z-20 flex items-center justify-center cursor-pointer"
            ...
            <div className="hidden md:flex items-center gap-8 text-[10px] font-bold uppercase tracking-[0.18em] h-full">
              <Link href="/products?category=Designer Sarees" className="hover:text-accent transition-colors flex items-center h-full">Sarees</Link>
              <Link href="/products?category=Wedding Lehengas" className="hover:text-accent transition-colors flex items-center h-full">Lehengas</Link>
              <Link href="/products?category=Western Dresses" className="hover:text-accent transition-colors flex items-center h-full">Dresses</Link>
              <Link href="/products" className="hover:text-accent transition-colors flex items-center h-full">All Couture</Link>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 md:gap-8 h-full">
            ...
            <div className="flex items-center gap-4 sm:gap-6 relative z-20 h-full">
              <Link href="/products" className="hover:text-accent transition-colors relative flex items-center h-full" title="Wishlist">
              ...
              <Link href="/cart" className="hover:text-accent transition-colors relative flex items-center h-full" title="Shopping Cart">
              ...
              <Link 
                ...
                className="hidden md:flex hover:text-accent transition-colors relative items-center h-full"
                title={currentUser ? `Profile: ${currentUser.name}` : 'Sign In'}
              >
                <User className={`w-5 h-5 ${currentUser ? 'text-primary' : ''}`} />
              </Link>
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-0 bg-[#1C1C1C]/20 backdrop-blur-[1px] z-[9998]"
                onClick={closeMobile}
              />
              ...
              <motion.div
                ...
                className="fixed top-0 left-0 bottom-0 w-[82vw] max-w-[82vw] md:w-80 bg-white z-[9999] flex flex-col shadow-2xl overflow-hidden rounded-none"
              >
                ...
                {/* Drawer Search Bar */}
                <div className="px-6 py-4 border-b border-[#E4DFDE]">
                  <form
                    onSubmit={(e) => {
                      handleSearchSubmit(e);
                      closeMobile();
                    }}
                    className="flex items-center gap-3 bg-[#FAFAF9] border border-[#E4DFDE] rounded-none px-4 py-2.5 focus-within:border-primary transition-colors"
                  >
                ...
                <div className="flex-1 overflow-y-auto py-6">
                  <div className="px-6 space-y-4">
                    ...
                    <Link
                      href="/"
                      onClick={closeMobile}
                      className={`flex items-center px-4 py-3.5 rounded-none text-[14px] font-semibold tracking-wide uppercase transition-all duration-200 ${
                        isActive('/') 
                          ? 'bg-[#F6F1EB] text-primary border-l-4 border-primary' 
                          : 'text-[#1C1C1C] hover:bg-[#FAFAF9]'
                      }`}
                    >
                      Home
                    </Link>
                    ...
                    <div className="space-y-1">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-4 py-3.5 rounded-none text-[14px] font-semibold tracking-wide uppercase text-[#1C1C1C] hover:bg-[#FAFAF9] transition-all duration-200 cursor-pointer"
                      >
                        <span>Categories</span>
                        {isDrawerCategoriesOpen ? <ChevronUp className="w-4 h-4 text-primary" /> : <ChevronDown className="w-4 h-4" />}
                      </button>
                      
                      {isDrawerCategoriesOpen && (
                        <div className="pl-4 border-l-2 border-[#E4DFDE] ml-4 mt-1 space-y-1">
                          {[
                            ...
                            <Link
                              ...
                              href={cat.href}
                              onClick={closeMobile}
                              className="block px-4 py-2.5 text-[13px] font-medium text-neutral-600 hover:text-primary hover:bg-[#FAFAF9] rounded-none transition-all duration-200"
                            >
                              {cat.name}
                            </Link>
                          ))}
                    ...
                    <Link
                      href="/products"
                      onClick={closeMobile}
                      className={`flex items-center justify-between px-4 py-3.5 rounded-none text-[14px] font-semibold tracking-wide uppercase transition-all duration-200 ${
                        isActive('/wishlist') 
                          ? 'bg-[#F6F1EB] text-primary border-l-4 border-primary' 
                          : 'text-[#1C1C1C] hover:bg-[#FAFAF9]'
                      }`}
                ...
                <div className="px-6 py-5 border-t border-[#E4DFDE] bg-white flex flex-col gap-4">
