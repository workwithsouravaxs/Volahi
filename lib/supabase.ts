...
const mapToProduct = (dbRow: any): Product => ({
  ...
  imagePosition: Array.isArray(dbRow.image_position) ? dbRow.image_position : (dbRow.image_position ? JSON.parse(dbRow.image_position) : undefined),
  image: dbRow.image,
...
const mapToDbProduct = (p: Omit<Product, 'reviews'>) => ({
  ...
  image_position: Array.isArray(p.imagePosition) ? p.imagePosition : null,
  image: p.image,
  stock: p.stock,
  sku: p.sku || null,
